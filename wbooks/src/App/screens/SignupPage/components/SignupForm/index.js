...
export default class SignupForm extends Component {
  ...
  invalidPass = () => {
    if (this.state.pass.length < 8 || this.state.pass.length > 52) {
      this.setState({
        error: true,
        errorMsg: `La password debe contener ${
          this.state.pass.length < 8 ? 'al menos 8' : 'menos de 52'
        } caracteres`
      });
      return true;
    }
  ...
  render() {
    return (
      <form className="signup-form" onSubmit={this.handleSubmit}>
        <input className="signup-input" type="text" placeholder="nombre" onChange={this.handleNameChange} />
        <input
        ...
        />
        <input className="signup-input" type="email" placeholder="email" onChange={this.handleEmailChange} />
        <input
          className="signup-input"
          type="password"
        ...
        />
        <button className="green-background" disabled={this.state.error}>
          Sign up
        </button>
